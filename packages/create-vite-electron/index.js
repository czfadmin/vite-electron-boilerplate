...
async function init() {
    ...
    const defaultProjectName =
        !targetDir && !"vite-electron-project" && argv._[0];
    let result = {};
    try {
        result = await prompts(
            [
                ...
                {
                    type:
                        _lang && LANGS.find(({ name }) => name === _lang)
                            ? null
                            : "select",
                    name: "lang",
                    message:
                        typeof _lang === "string" ? `Lang: ${_lang}` : "Lang",
                    initial: 0,
                    choices: LANGS.map((l) => {
                        const color = l.color;
                        return {
                ...
                },
                {
                    type:
                        _framework &&
                        FRAMEWORKS.map((f) => f.name).includes(_framework)
                            ? null
                            : "select",
                    name: "framework",
                    message:
    ...
    if (lang.name === "Typescript") {
        templateDir = path.join(
            __dirname,
            `../../templates/ts/${framework.name}`
        );
    } else {
        templateDir = path.join(
            __dirname,
            `../../templates/js/${framework.name}`
        );
    }
